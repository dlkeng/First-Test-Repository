...
#define PUSH_STACK(n)   memory_write_m(simulate->memory, REG_SP, (n) & 0xff); --REG_SP
#define PUSH_STACK16(n) simulate_push16_stm8(simulate, n)
#define PUSH_STACK24(n) simulate_push24_stm8(simulate, n)
#define POP_STACK()     memory_read_m(simulate->memory, ++REG_SP)
...
#define CY_TYPE_BIT 6
#define CY_ADD_TYPE 0
#define CY_SUB_TYPE BV(CY_TYPE_BIT)
...
#define GET_V()   ((simulate_stm8->reg_cc >> CC_V_FLAG) & 1)
#define GET_I1()  ((simulate_stm8->reg_cc >> CC_I1_FLAG) & 1)
#define GET_H()   ((simulate_stm8->reg_cc >> CC_H_FLAG) & 1)
#define GET_I0()  ((simulate_stm8->reg_cc >> CC_I0_FLAG) & 1)
#define GET_N()   ((simulate_stm8->reg_cc >> CC_N_FLAG) & 1)
#define GET_Z()   ((simulate_stm8->reg_cc >> CC_Z_FLAG) & 1)
#define GET_C()   ((simulate_stm8->reg_cc >> CC_C_FLAG) & 1)

#define SET_V()   simulate_stm8->reg_cc |= BV(CC_V_FLAG);
#define SET_I1()  simulate_stm8->reg_cc |= BV(CC_I1_FLAG);
#define SET_H()   simulate_stm8->reg_cc |= BV(CC_H_FLAG);
#define SET_I0()  simulate_stm8->reg_cc |= BV(CC_I0_FLAG);
#define SET_N()   simulate_stm8->reg_cc |= BV(CC_N_FLAG);
#define SET_Z()   simulate_stm8->reg_cc |= BV(CC_Z_FLAG);
#define SET_C()   simulate_stm8->reg_cc |= BV(CC_C_FLAG);

#define CLR_V()   simulate_stm8->reg_cc &= ~(BV(CC_V_FLAG));
#define CLR_I1()  simulate_stm8->reg_cc &= ~(BV(CC_I1_FLAG));
#define CLR_H()   simulate_stm8->reg_cc &= ~(BV(CC_H_FLAG));
#define CLR_I0()  simulate_stm8->reg_cc &= ~(BV(CC_I0_FLAG));
#define CLR_N()   simulate_stm8->reg_cc &= ~(BV(CC_N_FLAG));
#define CLR_Z()   simulate_stm8->reg_cc &= ~(BV(CC_Z_FLAG));
#define CLR_C()   simulate_stm8->reg_cc &= ~(BV(CC_C_FLAG));
...
{
  return -1;    // Use bprint or wprint to display RAM.
}
...
static void calculate_flags(struct _simulate * simulate, uint8_t flag_bits, uint16_t op1, uint16_t op2, uint16_t rslt, uint8_t bit_size)
{
  ...
  if (bit_size == SIZE_8BITS)
  {
    ...
    {
      if ((((op1 & 0x80) == 0) && ((op2 & 0x80) == 0) && ((rslt & 0x80) != 0)) || // values are positive && result is negative
          (((op1 & 0x80) != 0) && ((op2 & 0x80) != 0) && ((rslt & 0x80) == 0)))   // values are negative && result is positive
      {
        SET_V();
      }
      else
    ...
    {
      if ((flag_bits & BV(CY_TYPE_BIT)) != 0)   // [SUB/CP]
      {
  ...
  else if (bit_size == SIZE_16BITS)
  {
    ...
    if ((flag_bits & BV(CC_V_FLAG)) != 0)
    {
      if ((((op1 & 0x8000) == 0) && ((op2 & 0x8000) == 0) && ((rslt & 0x8000) != 0)) || // values are positive && result is negative
          (((op1 & 0x8000) != 0) && ((op2 & 0x8000) != 0) && ((rslt & 0x8000) == 0)))   // values are negative && result is positive
      {
        SET_V();
      }
      else
...
static int simulate_execute_stm8_op_common(struct _simulate * simulate, struct _table_stm8_opcodes * table_stm8, uint32_t eff_addr)
{
  ...
  switch (table_stm8->instr_enum)
  {
    case STM8_ADC:
      ...
      flag_bits = BV(CC_V_FLAG) | BV(CC_H_FLAG) | BV(CC_N_FLAG) | BV(CC_Z_FLAG) | BV(CC_C_FLAG) | CY_ADD_TYPE;
    ...
    case STM8_CP:
      rslt = REG_A - data;
      flag_bits = BV(CC_V_FLAG) | BV(CC_N_FLAG) | BV(CC_Z_FLAG) | BV(CC_C_FLAG) | CY_SUB_TYPE;
      calculate_flags(simulate, flag_bits, REG_A, data, rslt, SIZE_8BITS);
      return table_stm8->cycles_min;
    case STM8_CPW:
...
//    1 = had test failure
//    0 = no test failure
static int test_stm8_flags(struct _simulate * simulate,
    int test, uint8_t flag_bits, uint16_t op1, uint16_t op2, uint16_t rslt, int bit_size,
    int exp_V, int exp_H, int exp_N, int exp_Z, int exp_C)
{
  struct _simulate_stm8 * simulate_stm8 = (struct _simulate_stm8 *)simulate->context;
  int fail = 0;
  ...
  flag_bits |= BV(CC_V_FLAG) | BV(CC_H_FLAG) | BV(CC_N_FLAG) | BV(CC_Z_FLAG) | BV(CC_C_FLAG);

  calculate_flags(simulate, flag_bits, op1, op2, rslt, bit_size);
...
static int simulate_test_stm8_flags(struct _simulate * simulate, struct _table_stm8_opcodes * table_stm8)
{
  struct _simulate_stm8 * simulate_stm8 = (struct _simulate_stm8 *)simulate->context;
  uint8_t flag_bits;
  uint16_t op1;
  ...
  for (i = 0; i < 4; ++i)
  {
    switch (i)
    {
      ...
      case 2:
        ...
        exp_Z = 0;
        exp_C = 0;
        flag_bits = CY_SUB_TYPE;
        bit_size = SIZE_16BITS;
        break;
      case 3:
        op1 = 3;
        op2 = 2;
    ...
    fail_cnt += test_stm8_flags(simulate, test, flag_bits, op1, op2, rslt, bit_size, exp_V, exp_H, exp_N, exp_Z, exp_C);
    ++test;
  }
...
static int simulate_execute_stm8_op_none(struct _simulate * simulate, struct _table_stm8_opcodes * table_stm8)
{
  ...
  switch (table_stm8->instr_enum)
  {
    case STM8_BREAK:
#ifdef UNIT_TEST
      simulate_test_stm8_flags(simulate, table_stm8);
#endif
      SET_I1();
      SET_I0();
      return table_stm8->cycles_min;
  ...
  switch (table_stm8->instr_enum)
  {
    ...
    case STM8_SUBW:
      switch (table_stm8->dest)
      {
        case OP_REG_X:
          rslt = REG_X - data;
          flag_bits = BV(CC_V_FLAG) | BV(CC_H_FLAG) | BV(CC_N_FLAG) | BV(CC_Z_FLAG) | BV(CC_C_FLAG) | CY_SUB_TYPE;
          calculate_flags(simulate, flag_bits, REG_X, data, rslt, SIZE_16BITS);
          REG_X = rslt;
          return table_stm8->cycles_min;
        case OP_REG_Y:
    ...
    case STM8_ADDW:
      switch (table_stm8->dest)
      {
        ...
        case OP_REG_Y:
          data16 = READ_RAM16(eff_addr);
          rslt = REG_Y + data16;
          flag_bits = BV(CC_V_FLAG) | BV(CC_N_FLAG) | BV(CC_Z_FLAG) | BV(CC_C_FLAG) | CY_ADD_TYPE;
          calculate_flags(simulate, flag_bits, REG_Y, data16, rslt, SIZE_16BITS);
          REG_Y = rslt;
          return table_stm8->cycles_min;
        default:
